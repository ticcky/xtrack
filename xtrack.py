...
class LSTM:
    ...
    def build_accum_model(self, n_steps):
        ...
        #w_o = tt.matrix('W_o')
        curr_s = self.s0 = tt.vector('s0')

        def step(s, x):
            c = tt.tanh(tt.tensordot(self.params.w_x, x, [[1], [0]]))
            g_f =  tt.nnet.sigmoid(tt.tensordot(self.params.w_f, x, [[1], [0]]))
            g_i =  tt.nnet.sigmoid(tt.tensordot(self.params.w_i, x, [[1], [0]]))
            #g_o =  tt.nnet.sigmoid(tt.tensordot(w_o, x, [[0], [0]]))
        ...
        for i in range(n_steps):
            curr_x = tt.vector('x%d' % i)
            self.x.append(curr_x)
            curr_s = step(curr_s, curr_x)
...
def generate_data(n, n_steps):
    ...
    for _ in range(n):
        ...
        if random.random() > 0.5:
            ...
            res.append((np.asarray(x, dtype=np.float32), y))
        else:
            # Negative.
            x = []
...
def main():
    ...
    n_epochs = 1000
    # Generate some data.
    data = generate_data(100, n_steps)
    validation_data = generate_data(20, n_steps)

    lstm = LSTM()
    clf = lstm.build_clf_model(lstm.build_accum_model(n_steps))
    f_clf = function([lstm.s0] + lstm.x, clf)
    loss = lstm.build_loss(clf)
    f_loss = function([lstm.s0] + lstm.x + [lstm.tgt], loss)
    ...
    f_loss_prime = function([lstm.s0] + lstm.x + [lstm.tgt], loss_prime)
    ...
    for e in range(n_epochs):
        ...
        for x, y in data:
            ...
            g_point = f_loss_prime(*args)

            if g is None:
                g = np.zeros_like(g_point)
            g += g_point
        ...
        for i, (x, y) in enumerate(validation_data):
            ...
            total_loss += f_loss(*args) * 1.0 / len(validation_data)

            if e % 50 == 0:
                logging.info("%d: tgt(%.0f) clf(%.2f)" % (i, y, f_clf(*args[
                                                                       :-1])))

        logging.info("Validation loss: %.5f" % total_loss)
